export const articles = [
  {
    id: 1,
    title: 'UWAGA AKTUALIZACJA - Zimowa Sesja Egzaminacyjna 2021',
    date: '2022/01/18',
    imageUrl:
      'https://www.oyama-karate.eu/img/wo/6/37/egzaminy-369-obrazek_sredni_5107637.jpg',
    desc: 'WSZYSTKIE EGZAMINY SEKCJI: LIGOTA,  PODLESIE, PANEWNIKI I GLIWICE ZOSTAJĄ PRZENIESIONE NA STYCZEŃ 2022. ZA UTRUDNIENIA PRZEPRASZAMY.',
    text: (
      <>
        <p
          style={{
            fontWeight: 800,
            color: '#d9251c',
            width: '100%',
            textAlign: 'center'
          }}
        >
          AKTUALIZACJA
        </p>
        <p style={{ fontWeight: 800, color: '#d9251c' }}>
          WSZYSTKIE EGZAMINY SEKCJI: LIGOTA, PODLESIE, PANEWNIKI I GLIWICE
          ZOSTAJĄ PRZENIESIONE NA STYCZEŃ 2022. ZA UTRUDNIENIA PRZEPRASZAMY.
        </p>
        <br />
        <p style={{ textDecoration: 'underline', fontWeight: 800 }}>
          Egzaminy sesji odbędą się w następujących terminach:
        </p>
        <br />
        <p>
          <span style={{ textDecoration: 'underline', fontWeight: 800 }}>
            18.01.2022 r. (wtorek)
          </span>{' '}
          odbędzie się sesja egzaminacyjna na kolejne stopnie kyu.
        </p>
        <p style={{ fontWeight: 800 }}>
          Miejsce egzaminu: Katowice, SP.35, ul.Zielonogórska 23
        </p>
        <p>
          <span style={{ fontWeight: 800 }}>
            godz. 17:45 - wszystkie stopnie do 5kyu
          </span>{' '}
          Do tego egzaminu przystępują następujące sekcje z Katowic (MDK
          Podlesie, SP 35, SP 67) Należy być min 10 min wcześniej.
        </p>
        <br />
        <p>
          <span style={{ textDecoration: 'underline', fontWeight: 800 }}>
            14.01.2022 (piątek)
          </span>{' '}
          odbędzie się sesja egzaminacyjna na kolejne stopnie kyu.
        </p>
        <p style={{ fontWeight: 800 }}>
          Miejsce egzaminu: Gliwice, sala gimn. SP nr.28 Ul M.Srzody 4
        </p>
        <p>
          <span style={{ fontWeight: 800 }}>godz. 18.00 do 5 kyu włącznie</span>{' '}
          (maksymalnie na pomarańczowy pas (Ćwiczący w Gliwicach).
        </p>
        <br />
        <br />
        <p style={{ textDecoration: 'underline', fontWeight: 800 }}>
          (NALEŻY BYĆ 15 MINUT WCZEŚNIEJ)
        </p>
        <br />
        <p style={{ textDecoration: 'underline' }}>
          Do egzaminu mogą przystąpić tylko osoby posiadające opłaconą bieżącą
          składkę klubową za ostatnie 2 miesiące, co potwierdza wymaganą
          przepisami obecność zdającego na zajęciach.
        </p>
        <br />
        <p style={{ textDecoration: 'underline' }}>
          Wszystkich formalności należy dokonać najpóźniej do 13 grudnia.
        </p>
        <br />
        <p style={{ textDecoration: 'underline' }}>
          W przypadku pytań prosimy pytać instruktorów sekcji.
        </p>
      </>
    )
  },
  {
    id: 2,
    title: 'Obóz Zimowy Brenna 2022',
    date: '2021/11/30',
    imageUrl:
      'https://www.oyama-karate.eu/img/wo/6/36/246298282_4333657743396498_828872852779048482_n-obrazek_sredni_5107636.jpg',
    desc: 'Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne informacje są do pobrania w tym miejscu.',
    text: (
      <p>
        Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK
        "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne
        informacje są do pobrania w tym miejscu.{' '}
        <a
          style={{ color: '#d9251c', textDecoration: 'underline' }}
          href='https://www.oyama-karate.eu/tools/pobierz.php?typ=application/pdf&adr=/img/wo/6/36/a5107636/Brenna_2022.pdf&plik=Brenna_2022.pdf'
        >
          (link)
        </a>
      </p>
    )
  },
  {
    id: 3,
    title: 'Harmonogram rozpoczęcia Sezonu 2021/2022',
    date: '2021/08/30',
    imageUrl:
      'https://www.oyama-karate.eu/img/wo/5/44/Rozpoczecie-sezonu-2021-2022-Karate-Katowice-i-obrazek_sredni_5102544.jpg',
    desc: 'Harmonogram dotyczy sekcji Karate Katowice Podlesie, Katowice Ligota, Katowice Panewniki oraz Gliwice Centrum zarówno dla dzieci młodzieży oraz dorosłych.',
    text: (
      <>
        <p style={{ fontWeight: 800 }}>
          Harmonogram dotyczy sekcji Karate Katowice Podlesie, Katowice Ligota,
          Katowice Panewniki oraz Gliwice Centrum zarówno dla dzieci młodzieży
          oraz dorosłych.
        </p>
        <br />
        <br />
        <p>
          Aktualny sezon może odbyć się z większymi lub mniejszymi
          niespodziankami.
        </p>
        <p>
          Rozpoczynamy nowy sezon 2021-2021 według następującego harmonogramu:
        </p>
        <p>
          Nasze zajęcia będą prowadzone z zastosowaniem bieżących wytycznych
          epidemiologicznych.
        </p>
        <br />
        <br />
        <p style={{ width: '100%', textAlign: 'center', fontWeight: 800 }}>
          Nasz profil Fb będzie na bieżąco informował o wszystkich sprawach
          związanych z treningami Kaate zarówno w Katowicach jak i Gliwicach
        </p>
        <br />
        <p style={{ width: '100%', textAlign: 'center', fontWeight: 800 }}>
          https://www.facebook.com/Karate.Katowice.Gliwice
        </p>
        <br />
        <br />
        <p>Od poniedziałku 6 września powrócimy na sale według harmonogramu:</p>
        <br />
        <p style={{ textDecoration: 'underline', fontWeight: 600 }}>
          Treningi dla dzieci przedszkolnych:
        </p>
        <br />
        <p style={{ fontWeight: 800 }}>KATOWICE</p>
        <p>
          MDK Południe Katowice - Podlesie Ul. Sołtysia 25 - startuje
          poniedziałek 6 września 2021 o 15:20
        </p>
        <br />
        <p>
          Sp. nr 61 Katowice - Ligota Ul. Kołobrzeska 8 - startuje wtorek
          7.września 2021 o 16:30
        </p>
        <br />
        <p style={{ fontWeight: 800 }}>GLIWICE</p>
        <p>
          Sp. nr 28 Gliwice - centrum startuje w piątek 10 września 2021 o 17:20
        </p>
        <br />
        <br />
        <p style={{ textDecoration: 'underline', fontWeight: 600 }}>
          Treningi dla dzieci młodzieży oraz dorosłych:
        </p>
        <br />
        <p style={{ fontWeight: 800 }}>KATOWICE</p>
        <p>
          MDK Południe Katowice - Podlesie Ul. Sołtysia 25 – startuje w
          poniedziałek 6 września 2021 o 16:10
        </p>
        <br />
        <p>
          Sp. nr 67 Katowice - Panewniki Ul. Zielona 5 - startuje w poniedziałek
          6 września 2021 o 18:00
        </p>
        <p style={{ fontWeight: 600 }}>
          UWAGA W PONIEDZIAŁKI NA 18:00 w środy na 17:15.
        </p>
        <br />
        <p>
          Sp. nr 35 Katowice- Ligota Ul. Zielonogórska 23 - startuje we wtorek 7
          września 2021 o 17:45
        </p>
        <br />
        <br />
        <p style={{ fontWeight: 800 }}>GLIWICE</p>
        <p>
          Sp. nr 28 Gliwice - centrum startuje we wtorek 7 września 2021 o 18:00
        </p>
        <br />
        <br />
        <p style={{ width: '100%', textAlign: 'center', fontWeight: 800 }}>
          Aktualny harmonogram zajęć znajdą państwo klikając na poniszy link:
        </p>
        <br />
        <br />
        <p
          style={{
            width: '100%',
            textAlign: 'center',
            fontWeight: 800,
            color: '#d9251c',
            textDecoration: 'underline'
          }}
        >
          <a href='http://oyama-karate.gancle-studio.pl/schedule'>
            HARMONOGRAM ZAJĘĆ
          </a>
        </p>
        <br />
        <p style={{ width: '100%', textAlign: 'right' }}>
          Do zobaczenia na Dojo/Sali
        </p>
        <p style={{ width: '100%', textAlign: 'right' }}>OSU!</p>
      </>
    )
  },
  {
    id: 4,
    title: 'Obóz Zimowy Brenna 2022',
    date: '2021/11/30',
    imageUrl:
      'https://www.oyama-karate.eu/img/wo/6/36/246298282_4333657743396498_828872852779048482_n-obrazek_sredni_5107636.jpg',
    desc: 'Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne informacje są do pobrania w tym miejscu.',
    text: (
      <p>
        Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK
        "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne
        informacje są do pobrania w tym miejscu.{' '}
        <a
          style={{ color: '#d9251c', textDecoration: 'underline' }}
          href='https://www.oyama-karate.eu/tools/pobierz.php?typ=application/pdf&adr=/img/wo/6/36/a5107636/Brenna_2022.pdf&plik=Brenna_2022.pdf'
        >
          (link)
        </a>
      </p>
    )
  },
  {
    id: 5,
    title: 'Obóz Zimowy Brenna 2022',
    date: '2021/11/30',
    imageUrl:
      'https://www.oyama-karate.eu/img/wo/6/36/246298282_4333657743396498_828872852779048482_n-obrazek_sredni_5107636.jpg',
    desc: 'Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne informacje są do pobrania w tym miejscu.',
    text: (
      <p>
        Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK
        "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne
        informacje są do pobrania w tym miejscu.{' '}
        <a
          style={{ color: '#d9251c', textDecoration: 'underline' }}
          href='https://www.oyama-karate.eu/tools/pobierz.php?typ=application/pdf&adr=/img/wo/6/36/a5107636/Brenna_2022.pdf&plik=Brenna_2022.pdf'
        >
          (link)
        </a>
      </p>
    )
  },
  {
    id: 6,
    title: 'Obóz Zimowy Brenna 2022',
    date: '2021/11/30',
    imageUrl:
      'https://www.oyama-karate.eu/img/wo/6/36/246298282_4333657743396498_828872852779048482_n-obrazek_sredni_5107636.jpg',
    desc: 'Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne informacje są do pobrania w tym miejscu.',
    text: (
      <p>
        Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK
        "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne
        informacje są do pobrania w tym miejscu.{' '}
        <a
          style={{ color: '#d9251c', textDecoration: 'underline' }}
          href='https://www.oyama-karate.eu/tools/pobierz.php?typ=application/pdf&adr=/img/wo/6/36/a5107636/Brenna_2022.pdf&plik=Brenna_2022.pdf'
        >
          (link)
        </a>
      </p>
    )
  },
  {
    id: 7,
    title: 'Obóz Zimowy Brenna 2022',
    date: '2021/11/30',
    imageUrl:
      'https://www.oyama-karate.eu/img/wo/6/36/246298282_4333657743396498_828872852779048482_n-obrazek_sredni_5107636.jpg',
    desc: 'Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne informacje są do pobrania w tym miejscu.',
    text: (
      <p>
        Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK
        "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne
        informacje są do pobrania w tym miejscu.{' '}
        <a
          style={{ color: '#d9251c', textDecoration: 'underline' }}
          href='https://www.oyama-karate.eu/tools/pobierz.php?typ=application/pdf&adr=/img/wo/6/36/a5107636/Brenna_2022.pdf&plik=Brenna_2022.pdf'
        >
          (link)
        </a>
      </p>
    )
  },
  {
    id: 8,
    title: 'Obóz Zimowy Brenna 2022',
    date: '2021/11/30',
    imageUrl:
      'https://www.oyama-karate.eu/img/wo/6/36/246298282_4333657743396498_828872852779048482_n-obrazek_sredni_5107636.jpg',
    desc: 'Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne informacje są do pobrania w tym miejscu.',
    text: (
      <p>
        Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK
        "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne
        informacje są do pobrania w tym miejscu.{' '}
        <a
          style={{ color: '#d9251c', textDecoration: 'underline' }}
          href='https://www.oyama-karate.eu/tools/pobierz.php?typ=application/pdf&adr=/img/wo/6/36/a5107636/Brenna_2022.pdf&plik=Brenna_2022.pdf'
        >
          (link)
        </a>
      </p>
    )
  },
  {
    id: 9,
    title: 'Obóz Zimowy Brenna 2022',
    date: '2021/11/30',
    imageUrl:
      'https://www.oyama-karate.eu/img/wo/6/36/246298282_4333657743396498_828872852779048482_n-obrazek_sredni_5107636.jpg',
    desc: 'Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne informacje są do pobrania w tym miejscu.',
    text: (
      <p>
        Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK
        "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne
        informacje są do pobrania w tym miejscu.{' '}
        <a
          style={{ color: '#d9251c', textDecoration: 'underline' }}
          href='https://www.oyama-karate.eu/tools/pobierz.php?typ=application/pdf&adr=/img/wo/6/36/a5107636/Brenna_2022.pdf&plik=Brenna_2022.pdf'
        >
          (link)
        </a>
      </p>
    )
  },
  {
    id: 10,
    title: 'Obóz Zimowy Brenna 2022',
    date: '2021/11/30',
    imageUrl:
      'https://www.oyama-karate.eu/img/wo/6/36/246298282_4333657743396498_828872852779048482_n-obrazek_sredni_5107636.jpg',
    desc: 'Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne informacje są do pobrania w tym miejscu.',
    text: (
      <p>
        Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK
        "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne
        informacje są do pobrania w tym miejscu.{' '}
        <a
          style={{ color: '#d9251c', textDecoration: 'underline' }}
          href='https://www.oyama-karate.eu/tools/pobierz.php?typ=application/pdf&adr=/img/wo/6/36/a5107636/Brenna_2022.pdf&plik=Brenna_2022.pdf'
        >
          (link)
        </a>
      </p>
    )
  },
  {
    id: 11,
    title: 'Obóz Zimowy Brenna 2022',
    date: '2021/11/30',
    imageUrl:
      'https://www.oyama-karate.eu/img/wo/6/36/246298282_4333657743396498_828872852779048482_n-obrazek_sredni_5107636.jpg',
    desc: 'Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne informacje są do pobrania w tym miejscu.',
    text: (
      <p>
        Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK
        "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne
        informacje są do pobrania w tym miejscu.{' '}
        <a
          style={{ color: '#d9251c', textDecoration: 'underline' }}
          href='https://www.oyama-karate.eu/tools/pobierz.php?typ=application/pdf&adr=/img/wo/6/36/a5107636/Brenna_2022.pdf&plik=Brenna_2022.pdf'
        >
          (link)
        </a>
      </p>
    )
  },
  {
    id: 12,
    title: 'Obóz Zimowy Brenna 2022',
    date: '2021/11/30',
    imageUrl:
      'https://www.oyama-karate.eu/img/wo/6/36/246298282_4333657743396498_828872852779048482_n-obrazek_sredni_5107636.jpg',
    desc: 'Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne informacje są do pobrania w tym miejscu.',
    text: (
      <p>
        Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK
        "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne
        informacje są do pobrania w tym miejscu.{' '}
        <a
          style={{ color: '#d9251c', textDecoration: 'underline' }}
          href='https://www.oyama-karate.eu/tools/pobierz.php?typ=application/pdf&adr=/img/wo/6/36/a5107636/Brenna_2022.pdf&plik=Brenna_2022.pdf'
        >
          (link)
        </a>
      </p>
    )
  },
  {
    id: 13,
    title: 'Obóz Zimowy Brenna 2022',
    date: '2021/11/30',
    imageUrl:
      'https://www.oyama-karate.eu/img/wo/6/36/246298282_4333657743396498_828872852779048482_n-obrazek_sredni_5107636.jpg',
    desc: 'Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne informacje są do pobrania w tym miejscu.',
    text: (
      <p>
        Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK
        "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne
        informacje są do pobrania w tym miejscu.{' '}
        <a
          style={{ color: '#d9251c', textDecoration: 'underline' }}
          href='https://www.oyama-karate.eu/tools/pobierz.php?typ=application/pdf&adr=/img/wo/6/36/a5107636/Brenna_2022.pdf&plik=Brenna_2022.pdf'
        >
          (link)
        </a>
      </p>
    )
  },
  {
    id: 14,
    title: 'Obóz Zimowy Brenna 2022',
    date: '2021/11/30',
    imageUrl:
      'https://www.oyama-karate.eu/img/wo/6/36/246298282_4333657743396498_828872852779048482_n-obrazek_sredni_5107636.jpg',
    desc: 'Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne informacje są do pobrania w tym miejscu.',
    text: (
      <p>
        Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK
        "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne
        informacje są do pobrania w tym miejscu.{' '}
        <a
          style={{ color: '#d9251c', textDecoration: 'underline' }}
          href='https://www.oyama-karate.eu/tools/pobierz.php?typ=application/pdf&adr=/img/wo/6/36/a5107636/Brenna_2022.pdf&plik=Brenna_2022.pdf'
        >
          (link)
        </a>
      </p>
    )
  },
  {
    id: 15,
    title: 'Obóz Zimowy Brenna 2022',
    date: '2021/11/30',
    imageUrl:
      'https://www.oyama-karate.eu/img/wo/6/36/246298282_4333657743396498_828872852779048482_n-obrazek_sredni_5107636.jpg',
    desc: 'Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne informacje są do pobrania w tym miejscu.',
    text: (
      <p>
        Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK
        "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne
        informacje są do pobrania w tym miejscu.{' '}
        <a
          style={{ color: '#d9251c', textDecoration: 'underline' }}
          href='https://www.oyama-karate.eu/tools/pobierz.php?typ=application/pdf&adr=/img/wo/6/36/a5107636/Brenna_2022.pdf&plik=Brenna_2022.pdf'
        >
          (link)
        </a>
      </p>
    )
  },
  {
    id: 16,
    title: 'Obóz Zimowy Brenna 2022',
    date: '2021/11/30',
    imageUrl:
      'https://www.oyama-karate.eu/img/wo/6/36/246298282_4333657743396498_828872852779048482_n-obrazek_sredni_5107636.jpg',
    desc: 'Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne informacje są do pobrania w tym miejscu.',
    text: (
      <p>
        Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK
        "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne
        informacje są do pobrania w tym miejscu.{' '}
        <a
          style={{ color: '#d9251c', textDecoration: 'underline' }}
          href='https://www.oyama-karate.eu/tools/pobierz.php?typ=application/pdf&adr=/img/wo/6/36/a5107636/Brenna_2022.pdf&plik=Brenna_2022.pdf'
        >
          (link)
        </a>
      </p>
    )
  },
  {
    id: 17,
    title: 'Obóz Zimowy Brenna 2022',
    date: '2021/11/30',
    imageUrl:
      'https://www.oyama-karate.eu/img/wo/6/36/246298282_4333657743396498_828872852779048482_n-obrazek_sredni_5107636.jpg',
    desc: 'Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne informacje są do pobrania w tym miejscu.',
    text: (
      <p>
        Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK
        "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne
        informacje są do pobrania w tym miejscu.{' '}
        <a
          style={{ color: '#d9251c', textDecoration: 'underline' }}
          href='https://www.oyama-karate.eu/tools/pobierz.php?typ=application/pdf&adr=/img/wo/6/36/a5107636/Brenna_2022.pdf&plik=Brenna_2022.pdf'
        >
          (link)
        </a>
      </p>
    )
  },
  {
    id: 18,
    title: 'Obóz Zimowy Brenna 2022',
    date: '2021/11/30',
    imageUrl:
      'https://www.oyama-karate.eu/img/wo/6/36/246298282_4333657743396498_828872852779048482_n-obrazek_sredni_5107636.jpg',
    desc: 'Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne informacje są do pobrania w tym miejscu.',
    text: (
      <p>
        Serdecznie zapraszamy na obóz zimowy organizowany przez nasz Klub ŚKK
        "Goliat" w pierwszym tygodniu ferii zimowych. Wszystkie potrzebne
        informacje są do pobrania w tym miejscu.{' '}
        <a
          style={{ color: '#d9251c', textDecoration: 'underline' }}
          href='https://www.oyama-karate.eu/tools/pobierz.php?typ=application/pdf&adr=/img/wo/6/36/a5107636/Brenna_2022.pdf&plik=Brenna_2022.pdf'
        >
          (link)
        </a>
      </p>
    )
  }
];
